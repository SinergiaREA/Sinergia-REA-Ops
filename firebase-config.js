...
window.debugServiceWorker = async function() {
  console.log('🔍 Consultando estado del Service Worker...');
  
  if (!navigator.serviceWorker || !navigator.serviceWorker.controller) {
    console.error('❌ No hay Service Worker activo');
    return { error: 'No SW controller' };
  }

  return new Promise((resolve) => {
    const channel = new MessageChannel();
    const timeout = setTimeout(() => {
      console.error('❌ Timeout esperando respuesta del SW');
      resolve({ error: 'SW timeout' });
    }, 5000);

    channel.port1.onmessage = (event) => {
      clearTimeout(timeout);
      console.log('✅ Estado del Service Worker:', event.data);
      resolve(event.data);
    };
